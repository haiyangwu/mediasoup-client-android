...
jclass LazyGetClass(JNIEnv* env, const char* class_name, std::atomic<jclass>* atomic_class_id)
{
	...
	if (std::atomic_compare_exchange_strong(atomic_class_id, &cas_result, clazz.obj()))
	{
  ...
  std::atomic<jmethodID>* atomic_method_id)
{
	...
	if (value)
		return value;
	auto get_method_ptr =
	  type == MethodID::TYPE_STATIC ? &JNIEnv::GetStaticMethodID : &JNIEnv::GetMethodID;
	jmethodID id = (env->*get_method_ptr)(clazz, method_name, jni_signature);
	CHECK_EXCEPTION(env) << "error during GetMethodID: " << method_name << ", " << jni_signature;
